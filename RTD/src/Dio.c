...
#define DIO_SW_MAJOR_VERSION_C               4
#define DIO_SW_MINOR_VERSION_C               0
#define DIO_SW_PATCH_VERSION_C               2
...
{
    Std_ReturnType Valid = (Std_ReturnType)E_NOT_OK;
    uint8 u8ChannelGroupPort = pChannelGroup->port;
    ...
    if ((((pointerSizeType)(pChannelGroup)) >= ((pointerSizeType)Dio_ConfigPC.pChannelGroupList))                                  &&  \
        ...
        (u8ChannelGroupPort < DIO_NUM_PORTS_U16)                                                       &&  \
        (Dio_aAvailablePinsForWrite[u8ChannelGroupPort] != DIO_NO_AVAILABLE_CHANNELS_U16)              &&  \
...
)
{
    ...
    if ((((pointerSizeType)(pChannelGroup)) >= ((pointerSizeType)Dio_ConfigPC.pChannelGroupList))                                  &&  \
        (((pointerSizeType)(pChannelGroup)) <= (pointerSizeType)(&Dio_ConfigPC.pChannelGroupList[Dio_ConfigPC.u8NumChannelGroups - 1U])) &&  \
        (u8ChannelGroupPort < DIO_NUM_PORTS_U16)                                                       &&  \
        (Dio_aAvailablePinsForRead[u8ChannelGroupPort] != DIO_NO_AVAILABLE_CHANNELS_U16)               &&  \
        ((uint8)(pChannelGroup->u8offset) <= DIO_MAX_VALID_OFFSET_U8)
       )
    {
        Valid = (Std_ReturnType)E_OK;
...
)
{
    ...
    if ((Std_ReturnType)E_OK == Valid)
    {
        ...
        {
            if (Dio_pxChannelGroupsListForEachPartition[CoreId][GroupIndex] == ChannelGroupIdPtr)
            {
                GroupAlocated = 1;
            }
        }
