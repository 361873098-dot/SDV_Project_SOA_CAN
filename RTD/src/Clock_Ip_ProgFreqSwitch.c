...
#define CLOCK_IP_PROGFREQSWITCH_SW_PATCH_VERSION_C               2
...
#ifdef CLOCK_IP_CGM_X_PCFS_SDUR_DIVC_DIVE_DIVS
static uint32 HashPcfs[CLOCK_IP_DFS_INFO_SIZE];
#endif
...
static void Clock_Ip_CgmXPcfsSdurDivcDiveDivs(Clock_Ip_PcfsConfigType const *Config)
{
    ...
    if (NULL_PTR != Config)
    {
        ...
        if (HashPcfs[Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_PCFS_INDEX]] != ((((uint32)Config->ClockSourceFrequency) ^ ((uint32)Config->MaxAllowableIDDchange) ^ ((uint32)Config->Name)  ^ ((uint32)Config->SelectorName) ^ ((uint32)Config->StepDuration))))
        {
            HashPcfs[Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_PCFS_INDEX]] = ((((uint32)Config->ClockSourceFrequency) ^ ((uint32)Config->MaxAllowableIDDchange) ^ ((uint32)Config->Name)  ^ ((uint32)Config->SelectorName) ^ ((uint32)Config->StepDuration)));
