...
#endif
static void Power_Ip_ConfigPartCoreCofbReset(const Power_Ip_ModeConfigType * ModeConfigPtr, uint8 PartitionIndex);
...
static void Power_Ip_ConfigPartCoreCofbReset(const Power_Ip_ModeConfigType * ModeConfigPtr, uint8 PartitionIndex)
{
    ...
    Call_Power_Ip_MC_RGM_ModeConfig(ModeConfigPtr->McRgmModeConfigPtr, PartitionIndex);
    /* Check new mode configuration from MC_RGM. */
    Call_Power_Ip_MC_RGM_CheckModeConfig(ModeConfigPtr->McRgmModeConfigPtr, PartitionIndex);
}
...
static void Power_Ip_OnOffPartCoreCofb(const Power_Ip_ModeConfigType * ModeConfigPtr)
{
    ...
    for (PartitionIndex = 0U; PartitionIndex < POWER_IP_MAX_NUMBER_OF_PARTITIONS; ++PartitionIndex)
    {
        ...
        Power_Ip_MC_ME_EnablePartitionClock(ModeConfigPtr->McMeModeConfigPtr, PartitionIndex);

        /* Step 2: Enable interconnect interface of Software Reset Domain and Release the partition reset. */
        Call_Power_Ip_MC_RGM_EnableResetDomain(ModeConfigPtr->McRgmModeConfigPtr, PartitionIndex);

        /* Step 3: Disable Output safe stating base on configuration of McuPartitionClockEnable. */
        Power_Ip_MC_ME_DisablePartitionOutputSafe(ModeConfigPtr->McMeModeConfigPtr, PartitionIndex);
        ...
        Power_Ip_MC_ME_ConfigCoreCOFBClock(ModeConfigPtr->McMeModeConfigPtr, PartitionIndex);

        /* Step 5: Configure and check configure Core and COFB reset */
        Power_Ip_ConfigPartCoreCofbReset(ModeConfigPtr, PartitionIndex);

        /* Step 6: Disable interconnect interface of Software Reset Domain base on configuration of McuPartitionResetEnable. */
        Call_Power_Ip_MC_RGM_DisableResetDomain(ModeConfigPtr->McRgmModeConfigPtr, PartitionIndex);

        /* Step 7: Disable Partitions clock base on configuration of McuPartitionClockEnable.  */
        Power_Ip_MC_ME_DisablePartitionClock(ModeConfigPtr->McMeModeConfigPtr, PartitionIndex);

        /* Step 8: Enable Output safe stating base on configuration of McuPartitionClockEnable. */
        Power_Ip_MC_ME_EnablePartitionOutputSafe(ModeConfigPtr->McMeModeConfigPtr, PartitionIndex);
    }
}
