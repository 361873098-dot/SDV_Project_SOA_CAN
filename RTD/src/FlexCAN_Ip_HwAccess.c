...
#define FLEXCAN_IP_HWACCESS_SW_MAJOR_VERSION_C               4
#define FLEXCAN_IP_HWACCESS_SW_MINOR_VERSION_C               0
#define FLEXCAN_IP_HWACCESS_SW_PATCH_VERSION_C               2
/*==================================================================================================
*                                     FILE VERSION CHECKS
==================================================================================================*/
...
/* Check if current file and CAN header file are of the same Software version */
    ...
    #if ((FLEXCAN_IP_HWACCESS_AR_RELEASE_MAJOR_VERSION_C    != SCHM_CAN_AR_RELEASE_MAJOR_VERSION) || \
        (FLEXCAN_IP_HWACCESS_AR_RELEASE_MINOR_VERSION_C     != SCHM_CAN_AR_RELEASE_MINOR_VERSION) \
        )
        #error "AUTOSAR Version Numbers of FlexCAN_Ip_HwAccess.c and SchM_Can.h are different"
    #endif
#endif
/*==================================================================================================
...
void FlexCAN_ConfigTimestampModule(const Flexcan_Ip_TimeStampConfigType * config);
#else
static void FlexCAN_ConfigTimestampModule(const Flexcan_Ip_TimeStampConfigType * config);
#endif
...
static void FlexCAN_ClearRAM(FLEXCAN_Type * base)
{
    ...
    {
        RAM = (uint32 *)((uint32)base + (uint32)FLEXCAN_IP_FEATURE_EXP_RAM_OFFSET);
    ...
    uint32 ram_addr = (uint32)base + (uint32)FLEXCAN_IP_FEATURE_RAM_OFFSET;
...
#if (FLEXCAN_IP_FEATURE_HAS_EXPANDABLE_MEMORY == STD_ON)
    volatile uint32 * RAM_EXPANDED = (uint32*)((uint32)base + (uint32)FLEXCAN_IP_FEATURE_EXP_RAM_OFFSET);
#endif
...
Flexcan_Ip_StatusType FlexCAN_SetMaxMsgBuffNum(FLEXCAN_Type * base, uint32 maxMsgBuffNum)
{
    uint32 MsgBuffIdx;
    uint32 DataByte;
    const volatile uint32 * RAM = (uint32*)((uint32)base + (uint32)FLEXCAN_IP_FEATURE_RAM_OFFSET);
#if (STD_ON == FLEXCAN_IP_FEATURE_HAS_EXPANDABLE_MEMORY)
    const volatile uint32 * RAM_EXPANDED = (uint32*)((uint32)base + (uint32)FLEXCAN_IP_FEATURE_EXP_RAM_OFFSET);
    ...
    if (FlexCAN_IsExpandableMemoryAvailable(base))
    {
        ValEndRam = (uint32)&RAM_EXPANDED[FLEXCAN_IP_RAM1n_COUNT];
    }
    else
    {
        ValEndRam = (uint32)&RAM[(FlexCAN_GetMaxMbNum(base) * 4U)];
    }
#else
    ValEndRam = (uint32)&RAM[(FlexCAN_GetMaxMbNum(base) * 4U)];
#endif /* FLEXCAN_IP_FEATURE_HAS_EXPANDABLE_MEMORY */
                            ...
                            )
{
    ...
    volatile uint32 * FilterTable = (uint32 *)((uint32)base + (uint32)FLEXCAN_IP_RX_FIFO_FILTER_TABLE_OFFSET);
...
void FlexCAN_ReadRxFifo(const FLEXCAN_Type * base, Flexcan_Ip_MsgBuffType * rxFifo)
{
    ...
    volatile const uint32 * flexcan_mb = (uint32 *)((uint32)base + (uint32)FLEXCAN_IP_FEATURE_RAM_OFFSET);
    volatile const uint32 * flexcan_mb_id = &flexcan_mb[1];
    volatile const uint32 * flexcan_mb_data_32 = &flexcan_mb[2];
...
void FlexCAN_ReadEnhancedRxFifo(const FLEXCAN_Type * base, Flexcan_Ip_MsgBuffType * rxFifo)
{
    ...
    uint32 MbWord;
    uint8 Idhit_Offset;
    volatile const uint32 * Flexcan_Mb = (uint32 *)((uint32)base + (uint32)FLEXCAN_IP_FEATURE_ENHANCED_FIFO_RAM_OFFSET);
    volatile const uint32 * Flexcan_Mb_Id = &Flexcan_Mb[1];
    volatile const uint32 * Flexcan_Mb_Data_32 = &Flexcan_Mb[2];
    uint32 * MsgData_32 = NULL_PTR;
    ...
    /* Extract the IDHIT */
    rxFifo->id_hit = (uint8)(((Flexcan_Mb_Data_32[Idhit_Offset]) & FLEXCAN_IP_ENHANCED_IDHIT_MASK) >> FLEXCAN_IP_ENHANCED_IDHIT_SHIFT);
#if (FLEXCAN_IP_FEATURE_HAS_HR_TIMER == STD_ON)
    /* Extract the Time Stamp */
    if (FLEXCAN_IsHRTimeStampEnabled(base))
    {
...
{
    volatile const uint32 * Ram = (uint32 *)((uint32)base);
