...
#define CDD_PLATFORM_AR_RELEASE_MINOR_VERSION_C      4
#define CDD_PLATFORM_AR_RELEASE_REVISION_VERSION_C   0
#define CDD_PLATFORM_SW_MAJOR_VERSION_C              4
#define CDD_PLATFORM_SW_MINOR_VERSION_C              0
#define CDD_PLATFORM_SW_PATCH_VERSION_C              2
...
#if (PLATFORM_DEV_ERROR_DETECT == STD_ON)
    if (((sint32)INT_CTRL_IP_IRQ_MAX < (sint32)eIrqNumber) || \
...
Std_ReturnType Platform_SetIrqPriority(IRQn_Type eIrqNumber, uint8 u8Priority)
{
    ...
    if (((sint32)INT_CTRL_IP_IRQ_MAX < (sint32)eIrqNumber) || \
        ((sint32)INT_CTRL_IP_IRQ_MIN > (sint32)eIrqNumber) || \
        ((uint8)(1U << INT_CTRL_IP_NVIC_PRIO_BITS) <= u8Priority))
    {
...
Std_ReturnType Platform_GetIrqMonitorStatus(uint8 u8Channel, boolean *bLatencyExceeded)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {         
#endif
                *bLatencyExceeded = Platform_Ipw_GetIrqMonitorStatus(Platform_Config[u8CoreId] -> pIpwConfig,
                                                        u8Channel);
