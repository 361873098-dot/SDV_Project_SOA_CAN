...
def generate_header(db, basename: str, node: str) -> str:
    ...
    for msg in sorted(db.messages, key=lambda m: m.frame_id):
        ...
        for sig in msg.signals:
            ...
            if msg_is_tx:
                lines.append(f"{ctype} Can_R_W_signal_{sig.name}({ctype}* {sig.name});")
            elif msg_is_rx:
                lines.append(f"{ctype} Can_Get_Rx_signal_{sig.name}(void);")
        lines.append(f"")
    ...
    for msg in sorted(db.messages, key=lambda m: m.frame_id):
        ...
        for sig in msg.signals:
            ...
            if msg_is_tx:
                ...
                lines.append(f"/* --- {msg.name} (0x{msg.frame_id:03X}, {dtag}) --- if *{sig.name} is NULL this function is read else it's write */")
                lines.append(f"{ctype} Can_R_W_signal_{sig.name}({ctype}* {sig.name})")
                lines.append(f"{{")
                lines.append(f"    if({sig.name} == NULL)")
                lines.append(f"    {{")
                lines.append(f"        return ({global_var}.{sig.name});")
                lines.append(f"    }}")
                lines.append(f"    else")
                lines.append(f"    {{")
                lines.append(f"        {global_var}.{sig.name} = *{sig.name};")
                lines.append(f"        return ({global_var}.{sig.name});")
                lines.append(f"    }}")
                lines.append(f"}}")
                lines.append(f"")
                ...
                func_name = f"Can_Get_Rx_signal_{sig.name}"

                lines.append(f"{ctype} {func_name}(void)")
                lines.append(f"{{")
                lines.append(f"    return ({global_var}.{sig.name});")
                lines.append(f"}}")
                lines.append(f"")
